...
- **Manifest + validator**: `content/image-manifest.ts` tracks every planned image (status: `planned` → `generated` → `uploaded` → `approved`). `pnpm images:validate` checks shape, alt-text rules, no duplicates, and (when Cloudinary env is set) remote existence. `IMAGES_STRICT=1` makes the validator a hard production gate.
