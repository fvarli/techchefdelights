...
PGPASSWORD="$STRONG_PW" psql -h 127.0.0.1 -U tcd_staging -d techchefdelights_staging \
  ...
  -c 'SELECT COUNT(*) AS recipe_equipment FROM "RecipeEquipment";' \
  -c 'SELECT COUNT(*) FILTER (WHERE NOT required) AS optional_equipment FROM "RecipeEquipment";' \
  -c 'SELECT COUNT(*) FILTER (WHERE note IS NOT NULL) AS equipment_with_note FROM "RecipeEquipment";' \
...
#           27 recipe_equipment, 7 optional_equipment, 18 equipment_with_note,
#           function present
```
